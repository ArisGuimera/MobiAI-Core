...
You are reading this because `Skill(mobiai-create-pr)` was invoked — correct. Every HARD-GATE, phase checkpoint, and approval gate in this document is binding **because** the `Skill` tool was called. None of them bind if a future step, subagent, or different session reproduces this workflow from memory without another `Skill(mobiai-create-pr)` call.

If you need any of this skill's steps in another context, invoke `Skill(mobiai-create-pr)` again. Paraphrasing from memory is not activation.
