...
description: Use when the user shares a Firebase Crashlytics crash link, crash ID, or asks to investigate a Crashlytics issue in depth. Activate via the `Skill` tool; do not paraphrase this skill's workflow from memory.
license: MIT
...
You are reading this because `Skill(mobiai-crashlytics)` was invoked — correct. Every HARD-GATE, phase checkpoint, and approval gate in this document is binding **because** the `Skill` tool was called. None of them bind if a future step, subagent, or different session reproduces this workflow from memory without another `Skill(mobiai-crashlytics)` call.

If you need any of this skill's steps in another context, invoke `Skill(mobiai-crashlytics)` again. Paraphrasing from memory is not activation.
...
If none of these are available, fall back to the `mobiai-analyze-crash` skill with whatever information the user can provide manually.
